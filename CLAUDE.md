...
- Galaxy Watch 3 firmware `R850XXU1DWK2` (Tizen 5.5.0.2) ships a trimmed Developer-options menu without the ADB / Debug-over-Wi-Fi toggles. There is currently no confirmed way to enable `sdb` on this firmware build; install/test on this hardware is blocked at the device level until a workaround is found or hardware changes.
